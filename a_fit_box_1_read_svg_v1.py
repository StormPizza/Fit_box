...
def P3_convert_V_H_to_L(path_list_item, mark_list_item):
    ...
    for j in reversed(range(len(mark_list_item))):
        if mark_list_item[j].upper() == "H":  # there is x, must add y
            ...
            # mark_list_item.insert(j+1, "V")  # change this
            # mark_list_item.insert(j+1, "L")  # change this
            # bump = 0
...
def P5_translate_some(path_list_item):
    ...
    # for kk3 in reversed(range(int((len(path_list_item)/2)))):
    #     path_list_item[kk3*2] -= path_list_item[0]
        # path_list_item[(kk3*2)+1] -= path_list_item[1]

    # print(path_list_item, "will shift everything 1 by ", min(path_list_item), "P5")
    # print([int(i) for i in path_list_item], "BEFORE ")
    ...
    # print([int(i) for i in path_list_item], "AFTER ")


    # THE_MINIMUM = min(path_list_item)
    # if THE_MINIMUM < 0:
    #     for k3 in range(len(path_list_item)):
    #          path_list_item[k3] -= THE_MINIMUM - MARGIN

    # return path_list_item
    # path_list_item = [int(i) for i in path_list_item]
    return path_list_item


#/////////////////////////////////////////////////////////////////

def P__all_convert_svg_to_list(target_file):
